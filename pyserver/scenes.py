...
def shader_room(length=10, width=10, height=10):
    ...
    if ShaderLib is not None:
        ...
        # shader['uniforms']['tCube']['value'] = ["images/Park2/%s.jpg" % pos
        ...
        # shader['uniforms']['tCube']['value'] = ["images/MilkyWay/dark-s_%s.jpg" % pos
        #                                         for pos in ('px', 'nx', 'py', 'ny', 'pz', 'nz')]
...
def some_room(length=15.0, width=12.0, height=10.0):
    ...
    scene.add(Mesh(geometry=TextGeometry(text="Hello.  This scene was generated by the Python function 'scenes.some_room'", parameters={'size': 0.2, 'height': 0.02}),
...
def underwater_tomb(length=20.0, width=20.0, height=20.0):
    L, W, H = length, width, height
    yMin, yMax = -1.2, -1.2 + H
    xMin, xMax = -L/2, L/2
    zMin, zMax = -W/2, W/2
