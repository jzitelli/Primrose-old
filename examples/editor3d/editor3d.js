...
function editor3d() {
    ...
    function PrimroseDemo(err) {
        ...
        console.log("DOM sceneConfig: " + sceneConfig);
        ...
        function showConfig(editor) {
            log("sceneConfig = " + sceneConfig, 0xff0000, editor);
        }
...
//         function textGeomLog(msg, color, pos) {
//             // TODO: one geom per *unique* line
//             textgeom_log_buffer.push(msg);
//             var size = 0.5;
//             var height = 0.5 / 20;
...
//                 }
//                 mesh.position.y += (textgeom_log_meshes.length - i - 1) * 1.75 * size;
//             }
//         }
...
//             shading: THREE.FlatShading
//         });
//         var vrMenuMesh = new THREE.Group();
//         var vrMenuOptions = [];
//         // log enable periodic updates:
//         // fps (last 200 frames, last 100 frames, last 50 frames, last 25 frames, last 10 frames) 
//         // fps measure / profiler / optimizer
...
//         // menu shortcut label
//         // menu shortcut key
//         function vrMenuCreate(options) {
//             // TODO: how to slice arrays in javascript
//             for (var i = 0; i < Math.min(4, options.length); ++i) {
//                 var geom = new THREE.TextGeometry("option value: " + options[i].value, {
//                     size: 0.5,
//                     height: 0.05,
//                     font: 'janda manatee solid',
//                     weight: 'normal'
...
//         ctrls = findEverything();
//         console.log("found everything: " + ctrls);

//         sceneConfig = getData(ctrls.sceneConfig.id);
//         console.log("DOM scene config: " + sceneConfig);
//         console.log("sceneConfig.floor_size: " + sceneConfig.floor_size);

//         vrLog = sceneConfig.vr_log || $.QueryString["vr_log"];
...
//             }); //.replace('\n', '%0A')})
//             if (vrLog) {
//                 var pos = new THREE.Vector3(body.position);
//                 pos.x -= 10;
//                 pos.z -= 10;
//                 pos.y = 2;
//                 textGeomLog(msg, color || 0x00ff00, pos);
//             }
//             if (editor) {
//                 editor.overwriteText(msg);
//                 editor.drawText();
...
//                 }
//             }
//             vrMenuCreate($("option").get());
//             PrimroseDemo();
//         }

//         function saveScene(name) {
//             sceneConfig.editors = editors;
//             var data = {
//                 name: name,
//                 rev: rev,
//                 sceneConfig: sceneConfig
//             };
//             // for (var i = 0; i < editors.length; ++i) {
//             //     sceneConfig.editoreditor in editors
//             $.post("/save", data).
//             done(
//                 function(data) {
//                     log("saved " + name + " (version " + rev + ") to " + data.filename);
...
//             var camera = new THREE.PerspectiveCamera(50, ctrls.output.width /
//                 ctrls.output.height, 0.1, 1000);
//             var fakeCamera = new THREE.PerspectiveCamera(50, ctrls.output.width /
//                 ctrls.output.height, 0.001, 1000);
//             scene.add(fakeCamera);
...
//                 editors.push(textbox);
//                 editor_geoms.push(flatEditor);
//                 // eval editor.success?? (python providing javascript callback)
//                 var success = function(data) {};
//                 if (editor.filename) {
//                     $.ajax({
//                         url: "/read?filename=" + editor.filename
//                     }).
//                     done(function(data) {
//                         editor.file = data.value;
//                         sceneConfig.editors.push(editor);
//                         textbox.overwriteText(data.value);
//                         textbox.drawText();
//                         log("loaded " + data.args.filename);
//                         success(data);
